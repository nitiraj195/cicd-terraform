    ...
    stages {
        stage('Plan') {
            steps {
                ...
                sh 'terraform init --backend=true --backend-config=\"bucket=my-dev-env1905\" --backend-config=\"key=terraform/cicd.tfstate\" --backend-config=\"region=us-east-1\" -lock=false'
